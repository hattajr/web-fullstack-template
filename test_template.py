    ...
    try:
        ...
        if result.returncode != 0:
            ...
            if with_db and "Failed to connect to PostgreSQL server" in result.stdout:
                print(
                    "⚠️  Database connection test failed (expected without real PostgreSQL)"
                )
                ...
                print("❌ Cookiecutter failed unexpectedly!")
                print(f"   stdout: {result.stdout}")
                print(f"   stderr: {result.stderr}")
                return False

        if not generated_project.exists():
            print(f"❌ Generated project not found at {generated_project}")
            return False
        ...
        if with_db:
            ...
            if "from db.connection import get_connection" in content:
                print("  ✅ Database import found in main.py")
            else:
                print("  ❌ Database import NOT found in main.py")
                return False

            db_file = generated_project / "app/db/connection.py"
            if db_file.exists():
                print("  ✅ db/connection.py exists")
            else:
                print("  ❌ db/connection.py - MISSING!")
                return False
        else:
            ...
            if "from db.connection import get_connection" not in content:
                print("  ✅ No database import in main.py (as expected)")
            else:
                print("  ❌ Database import found in main.py (should not be there!)")
                return False
        ...
        try:
            ...
            print(
                "  ⚠️  Dependency installation failed (might be expected in test environment)"
            )
            print(f"     Error: {e}")
        ...
        if pyproject.exists():
            ...
            if project_name.replace("-", "_") in content or project_name in content:
                print("  ✅ Project name correctly set in pyproject.toml")
            else:
                print("  ⚠️  Project name might not be correctly set")
